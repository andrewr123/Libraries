/*
  ...
  Feb 2013 - AR amended to use 'wakeup' capabilities to keep time current, rather than in now()
*/
...
time_t now(){					// AR - removed calculation of time from here - just call getTimePtr
	...
	time_t t = 0;
  	...
  	if(getTimePtr != 0){
		...
		Status = (t) ? timeSet : (Status == timeNotSet) ?  timeNotSet : timeNeedsSync;        
  	}
