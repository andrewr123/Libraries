...
#endif
{
  ...
  // Select the aaa hardware address for this chip by tying pins A0, A1, and A2 to either 5v or GND.
  // We need to enable HAEN on all connected devices before we can address them individually
...
void MCP23S17::intMode (byte pin, byte IOCMode) {
	...
	switch (IOCMode) {				// Set appropriate control flags in MCP and (if RISING/FALLING) locally
		...
		case RISING:
			...
			intDirection |= (1 << pin);		// Was risen change if INTCAP is set
			break;
		case FALLING:
			write_addr (INTCON, read_addr(INTCON) & ~(1 << pin));	// IOCPREV
			intTest |= (1 << pin);		// Test for direction against INTCAP needed after interrupt
			intDirection &= ~(1 << pin);	// Was fallen change if INTCAP is clear
			break;
		case WHILEHIGH:
...
void MCP23S17::printReg(String name) {
  Serial.print("Device ");
  Serial.println(name);
  const String registers[] = { "IODIR", "IPOL", "GPINTEN", "DEFVAL", "INTCON", "IOCON", "GPPU", "INTF", "INTCAP", "GPIO", "OLAT" };
  unsigned int regVal;
  for (int i = 0; i < 11; i++) {
    Serial.print("Register ");
    Serial.print(registers[i]);
    Serial.print(" = ");
    regVal = read_addr(i * 2);
    if (i != 5) {
      Serial.print(regVal >> 8, BIN);
      Serial.print(" ");
    }
...
unsigned int MCP23S17::getGPINTEN() {
  return read_addr(GPINTEN);
}
