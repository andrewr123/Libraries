...
//const static uint8_t FALLING = 2;				// INTCON == 0 plus interpretation: raise interrupt if low after high (defined in arduino.h)
//const static uint8_t RISING = 3;				// INTCON == 0 plus interpretation: raise interrupt if high after low (defined in arduino.h)
...
class MCP23S17
{
  public:
    ...
    // Debugging
    void printReg(String name);
    unsigned int getGPINTEN();
  ...
  protected:
    ...
    #endif
    uint16_t intTest;					// 16 bits: 0 = no interpretation (accept interrupt), 1 = interpret by comparing INTCAP against intDirection
    uint16_t intDirection;		// 16 bits: Set in intEnable, then (if intTest == 1) used to interpret INTCAP when IOCMode == RISING (1) or FALLING (0)
    uint16_t intEnablePins;		// 16 bits: Set/unset in intEnable/intDisable; amended in intValid.  Copy of GPINTEN
