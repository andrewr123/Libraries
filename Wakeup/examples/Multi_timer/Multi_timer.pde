 /*
    ...
    v1.1 - modified to align with v1.1 of wakeup
...
void setup(void) {
  ...
  wakeup.wakeMeAfter(callbackSimple, 100, NULL, TREAT_AS_ISR | REPEAT_COUNT);    // Toggle LED 13 every 100mS
  
  // Example 2 - set timer to wake up main loop every 1000ms
  wakeup.wakeMeAfter(stopCounter, 1000, (void*)&keepCounting, TREAT_AS_ISR | REPEAT_COUNT);    // keepCounting is volatile, so need to cast 
  ...
  wakeup.wakeMeAfter(callbackComplex, 5, NULL, TREAT_AS_NORMAL | UNITS_SECONDS);
  ...
  for (int i = 0; i < MAXSLEEPERS; i++) {  
    ...
    contextStruct[i].interval = interval * 1000;
   
    if (wakeup.wakeMeAfter(callbackWithStruct, interval, &contextStruct[i], TREAT_AS_NORMAL | UNITS_SECONDS) == false) {
