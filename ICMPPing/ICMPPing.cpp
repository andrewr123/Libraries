 ...
 *
 * Apr 13 - Modified by AR to avoid need to specify socket number.  Finds first free socket (code lifted from EthernetUDP)
 */
...
{
	// AR - add code to lookup free socket - lifted from EthernetUDP
	if (socket == MAX_SOCK_NUM) {	
	  	...
	  	if (socket == MAX_SOCK_NUM) return 0;
  }
  ...
  W5100.execCmdSn(socket, Sock_OPEN);
	...
	for (int i=0; i<nRetries; ++i)
	{
		...
		if (waitForEchoReply())
		{
			...
			if (result) sprintf(result, "%d|%ld|%d", REQ_DATASIZE, time, TTL);
			rval = true;
			break;
		}
		else
		{
			if (result) sprintf(result, "T");
		}
	}
	W5100.execCmdSn(socket, Sock_CLOSE);
	W5100.writeSnIR(socket, 0xFF);
